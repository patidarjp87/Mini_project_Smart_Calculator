...
operations={'*':mul,'-':sub,'/':div,'//':div2,'MUL':mul,'DIV':div,'SUB':sub,'+':add,'HCF':hcfoftwo,'LCM':lcmoftwo,'PLUS':add,'ADD':add,'ADDITION':add,'SUM':add,'MINUS':sub,'SUBTRACT':sub,'DIFFERENTIATE':sub,'DIFFERENCE':sub,'PRODUCT':mul,'MULTIPLY':mul,'DIVIDE':div,'DIVISION':div}
commands={'CONTACT':contact,'WANT':want,'CHARGE':want,'FEE':want,'LEARN':contact,'PURPOSE':purpose,'MISSION':purpose,'VISION':purpose,'USE':use,'DO':use,'WORK':use,'ABILITY':use,'DEVLOP':made,'MADE':made,'HOW':use,'WHO':made,'DEVLOPER':made,'DEVLOP':made,'NAME':myname,'CLOSE':end,'END':end,'TERMINATE':end,'EXIT':end}
